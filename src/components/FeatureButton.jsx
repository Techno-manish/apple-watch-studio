...
import { setBand, setFace, setSize } from "@/redux/slices/watchSlice";
import { setFeatureButtonOpen } from "@/redux/slices/uiSlice";
import { useAppDispatch, useAppSelector } from "@/lib/hooks";
import { collections } from "@/data/collections";

const FeatureButtons = () => {
  const dispatch = useAppDispatch();
  const { collection, size, face, band } = useAppSelector(
    (state) => state.watch
  );
      ...
      <div className="min-h-[35px] overflow-x-auto pb-[5px] flex justify-center w-full">
        <div className="bg-[#e8e8ed] space-x-2 rounded-full items-center flex text-[#1d1d1f] border-none mx-[6px] px-4 sm:px-6 font-proTextRegular text-[14px] sm:text-[16px] md:text-[17px] tracking-[-.022em]">
          ...
          >
            {openButton === "Size" ? (
              filteredCases &&
              filteredCases?.sizes?.map((option) => (
                <button
                  key={option.id}
        ...
        <div className="bg-[#e8e8ed] space-x-2 rounded-full  items-center flex  text-[#1d1d1f] border-none  mx-[6px] px-4 sm:px-6  font-proTextRegular text-[14px] sm:text-[16px] md:text-[17px] tracking-[-.022em] ">
          ...
          {openButton === "Case" ? (
            filteredCases?.cases.map((mainCase) => {
              return (
                <button
                  key={mainCase.id}
                  onClick={() => {
        ...
        <div className="bg-[#e8e8ed] space-x-2 rounded-full  items-center flex  text-[#1d1d1f] border-none  mx-[6px] px-4 sm:px-6  font-proTextRegular text-[14px] sm:text-[16px] md:text-[17px] tracking-[-.022em] ">
          <div className="inline-block">
            ...
            >
              ...
              <path
                d="m9.5 22.5a.5.5 0 0 1 -.5.5h-8a.5.5 0 1 1 0-1h.015a.485.485 0 0 0 .485-.485v-6.2216a4.5231 4.5231 0 0 0 1 .9448v5.2768a1.4779 1.4779 0 0 1 -.0813.485h5.1627a1.4758 1.4758 0 0 1 -.0814-.485v-5.2768a4.5209 4.5209 0 0 0 1-.9448v6.2216a.4851.4851 0 0 0 .4851.485h.0149a.5.5 0 0 1 .5.5zm-1.9194-19.5h-5.1621a1.4732 1.4732 0 0 1 .0815.485v9.015a2.5 2.5 0 0 0 5 0v-9.015a1.4873 1.4873 0 0 1 .0806-.485m1.4194-1a.5.5 0 0 1 .5.5.5.5 0 0 1 -.5.5h-.015a.488.488 0 0 0-.485.485v8.377a2.4438 2.4438 0 0 0-1-.9448v-7.6356a.5.5 0 0 1 .5-.5h3.5z"
                fill="#1d1d1f"
              ></path>
            </svg>
          </div>
          {openButton === "Band" ? (
            filteredCases?.bands.map((bandOption) => (
              <button
                key={bandOption.id}
                onClick={() => dispatch(setBand(bandOption))}
                className={`my-[5px] min-h-[20px] text-[16px] md:text-[17px]  align-middle text-[#1d1d1f] py-[5px] ${
                  band?.id === bandOption.id
                    ? "font-bold"
                    : "font-proTextRegular"
                }`}
              >
