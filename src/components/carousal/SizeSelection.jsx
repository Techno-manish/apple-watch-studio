...
import { setSize } from "@/redux/slices/watchSlice";
...
const SizeSelection = () => {
  ...
  const { collection, size } = useAppSelector((state) => state.watch);
  ...
  const snapToNearestWatch = () => {
    ...
    const sizes = collections.find((opt) => opt.id === collection)?.sizes;
    const selectedSize = sizes?.find((s) => s.id === watchId);
